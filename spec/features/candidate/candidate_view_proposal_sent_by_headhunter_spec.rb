...
feature 'Candidate view proposal' do
  ...
  scenario 'candidate must not see proposals of another candidate' do
    ...
    entry = create(:entry, candidate: candidate, vacancy: vacancy, status: :avaiable)
    create(:proposal, entry: entry, candidate: candidate, headhunter: headhunter)
    candidate2 = create(:candidate, status: :complete, email: 'test@123')
    create(:profile, candidate: candidate2, social_name: 'Leticia Alves')
    entry2 = create(:entry, candidate: candidate2, vacancy: vacancy, status: :avaiable)
    create(:proposal, entry: entry2, candidate: candidate2, headhunter: headhunter)
